    ...
    context "on #{os}:" do
      ...
      context 'Test /etc/pam.d/sshd oath through ssh' do
        ...
        it 'test user exclusion' do
          on(server, %q(echo 'test_user' >> /etc/liboath/exclude_users.oath))
          on(client, "/usr/local/bin/ssh_test_script #{test_user} #{os}-server #{password}")
          # Clean up test_user out of exclude_users file
          on(server, %q(echo 'vagrant' > /etc/liboath/exclude_users.oath))
          on(server, %q(echo 'root' >> /etc/liboath/exclude_users.oath))
        end
        
        it 'test group exclusion' do
          on(server, 'groupadd test_group')
          on(server, %q(echo 'test_group' >> /etc/liboath/exclude_groups.oath'))
          on(server, "usermod -G test_group #{test_user}")
          on(client, "/usr/local/bin/ssh_test_script #{test_user} #{os}-server #{password}")
          # Clean up test_user out of exclude_groups file
          on(server, %q(echo '' > /etc/liboath/exclude_groups.oath))
        end
      end
    end
  end
